...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, holiday|
      ...
      holiday.each do |day, supply|
          ...
          day_name = day_array.collect {|name| name.capitalize}
          holiday_name = day_name.join(" ")
          supply_list = "  #{holiday_name}: #{supply[0]}"
          i = 1
          while supply.length > i
              supply_list = supply_list + ", #{supply[i]}"
              i += 1
              end
          puts supply_list
